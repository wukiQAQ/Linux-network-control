...
internal/action/         # 白名单运维动作：命令预览、参数校验、危险确认与审计
...
- **V0.8.0**：左侧按分类新增「系统 / 网络 / 服务 / 系统日志 / 文件」运维入口；点动作会弹出窗口**先显示将在 Linux 上执行的完整命令**，填参数后命令实时更新；可重启/停止服务（危险动作需二次确认）；所有执行写入审计记录，采集端提供动作目录与执行接口
- **V0.7.1**：历史曲线只保留折线 / 面积两种样式；左右坐标轴上限可手动填写（留空为自动，自动模式按档位取整不再跳动）；采集端上报版本与能力，旧版服务端会置灰抓包按钮并提示更新
...
| V0.7.1 ✅ | 移除柱状图、坐标轴可手动设定、抓包失败给出明确提示（已实现） |
| V0.8.0 ✅ | 白名单运维动作（系统/网络/服务/日志/文件）、命令预览、危险确认与审计（已实现） |
| V0.8.1 ✅ | 曲线坐标轴默认锁定、服务端版本与能力展示（已实现） |
| V0.9.0 ✅ | 文件通道：白名单目录浏览 + 下载到 Windows（已实现） |
| V0.10.0 ✅ | 实时推送通道（SSE）+ 客户端自动重连与轮询降级（已实现） |
| V0.11.0 ✅ | 多机主机列表（在线探测 + 版本/能力 + 一键切换）（已实现） |
| V0.12.0 ✅ | 告警触发自动导出抓包（已实现） |
| V0.13.0 ✅ | 协议纵深：IPv6 解析（含扩展头）（已实现） |
| V0.14.0 ✅ | TOP N 与协议分布（已实现） |
| V0.12.1 ✅ | 客户端曲线显示平滑（关/轻/中/强，默认轻） |
...
| V0.12.5 ✅ | 客户端：左侧统一「Linux 指令」面板（全部白名单命令集中展示 + 复制全部指令）（已实现） |
| V0.18.0 ✅ | 采集端采集过滤：`[capture] filter`（pcap 常用子集、用户态匹配、非法表达式启动即报错）（已实现） |
...
| V0.21.0 ✅ | 采集端多队列 CPU 绑定（`[capture] cpu_pin` + `capture.cpupin` 能力）、连接指南与采集基线脚本（已实现） |
...
- [功能清单与说明](docs/功能清单.md)：当前已实现功能的完整统计（版本 / 能力 / 接口 / 动作 / 配置 / 测试）
- [连接指南](docs/连接指南.md)：Windows ↔ Linux 虚拟机怎么连（网络模式 / 上传 / 配置 / 客户端 / 排错）
